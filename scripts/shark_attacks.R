...
shark_attacks <- read_csv(here("data", "shark_attacks_cln.csv"), trim_ws = TRUE, skip_empty_rows = TRUE) %>% 
  as_tibble() %>% 
  clean_names()
...
shark_species <- "STARRY SMOOTHHOUND|SOUPFIN|LEOPARD|THRESHER|BROWN|COCKTAIL|COW|C. ALBIMARGINATUS|BANJO|CATSHARK|SPEAR-EYE|GROUND|BLUE POINTER|TAWNY NURSE|WHITETIP REEF|OCEAN WHITETIP|TIGER|BULL|NURSE|BRONZE WHALER|WHALER|WHALE|WOBBEGONG|SHORTFIN MAKO|MAKO|RAGGEDTOOTH|BLACKTIP REEF|BLACKTIP|GREY NURSE|LEMON|BLUE|HAMMERHEAD|SANDTIGER|ZAMBESI|ZAMBEZI|GREY REEF|SPINNER|CARIBBEAN REEF|BASKING|REEF|SANDBAR|SAND|ANGEL|CARPET|DUSKY|PORBEAGLE|GALAPAGOS|DOG|7-GILL|BROADNOSE SEVENGILL|SEVENGILL|SEVEN-GILL|COOKIECUTTER|SALMON|COPPER|GOBLIN|SHOVELNOSE|SILKY|SILVERTIP|GRAY|GREY|WHITE"

maybes <- " OR |\\?|POSSIBLY|POSSIBILY|THOUGHT|SAID"
